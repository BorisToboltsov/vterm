...
- 🤖 **ИИ-ассистент** (опционально, выключен по умолчанию, **в разработке**): чат во
  ...
  выбору; весь трафик — через бэкенд только на ваш эндпоинт, ключи в keychain.
  ...
  **«Выполнить»** (режимы: только предлагать / подтверждать каждую / авто на не-прод), запуск
  пишется в запись сессии как аудит. Из записи сессии ИИ собирает **план-runbook** или **скрипт
  `sh`/Ansible**, открывая его в редакторе с линтом.
- 🎨 **UX**: темы (терминал + UI), командная палитра ⌘K, доступность (a11y),
  i18n (English/Русский), бэкап настроек, **офлайн по умолчанию** (кроме SSH и опц. ИИ).
