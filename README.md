...
> ⚙️ **Текущий статус: Фазы 0–12 реализованы.** SSH-терминал
...
> ИИ-экспорт, автозапись прод-серверов. **Фаза 12 (продвинутый SFTP):** встроенный редактор
> конфигов (CodeMirror, 50+ языков, diff/линт/markdown-превью), правка файлов на сервере и
> локально, sudo-правка root-конфигов, синхронизация папок по SHA-256, grep по содержимому.
...
- 📈 **Страница подробного мониторинга** — KPI-дашборд: вверху блок **«Система»**
  (хост/ОС/ядро/uptime/IP/время/ядра) и ряд **KPI-плиток** (CPU, RAM, диск, load,
  температура, сеть) с радиальным индикатором и цветом порога, ниже — детальные
  ...
  **таблицей топ-процессов**, **здоровьем системы** (сбойные systemd-юниты, слушающие
  ...
  дисков** и счётчик OOM-kills — подгружаются один раз при открытии). **Два режима плотности** — компактный
  (только плитки) и подробный; клик по плитке разворачивает и ведёт к её секции.
...
### Реализовано (Фаза 12) и запланировано (Фазы 13–14)
...
- 🖼️ **Продуктовый README**: скриншоты/гифки, бейджи (Фаза 14).
