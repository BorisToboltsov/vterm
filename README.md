...
- 📝 **Редактор конфигов** (CodeMirror) с подсветкой, diff перед сохранением,
  ...
  (и `lm-sensors` для температуры) — раздел «Серверные инструменты» в настройках: статус на сервере
  + установка в один клик (в терминал или через sudo), с учётом пакетного менеджера дистрибутива.
