...
- 🔎 **Поиск по всему буферу терминала** — не только по видимому экрану, а по всему
  ...
  ↑/↓) и кнопкой **«копировать совпадение с контекстом»** (строка ±5). Совпадения
...
- 🎨 **Подсветка логов по правилам (regex):** настраивается в настройках («Логи и
  ...
  регистра. Из коробки: `ERROR/FATAL` — красным, `WARNING` — жёлтым, IP-адреса —
  синим. Подсветка идёт **цветами текущей темы** и **только в обычном выводе**
  (полноэкранные программы вроде `vim`/`htop` не затрагиваются).
...
- ⚙️ **Раздел настроек «Логи и текст»** с **мастер-переключателем**: один чекбокс
  ...
  (плюс отдельные тумблеры по фичам: поиск, подсветка).

### Запланировано (Фазы 10–14)
- 📜 **Умные логи** (Фаза 10): regex-подсветка и табличный JSON-просмотр
  (поиск по буферу — **уже реализован**, см. выше).
