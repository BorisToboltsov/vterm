...
- `actions/tooltip.ts` (Фаза 20.17) — экшен подсказки: пузырёк с `role="tooltip"`
  появляется по `mouseenter`/`focus` и **портируется в `document.body`** (не внутри
  триггера), убирается по `mouseleave`/`blur`; пустой текст → ничего; `update()`
  переименовывает открытый пузырёк; `destroy()` убирает его и снимает слушатели;
...
- `HelpPanel.test.ts` — вкладки Help/Инструкция/About, таблица хоткеев (в т.ч. строка
  ⌘/Ctrl+T «новая вкладка», Фаза 20.15), версия из
...
- `ServerFormModal.test.ts` (Фаза 20.5) — валидация обязательных полей формы сервера
  ...
  **порт** — очистка блокирует сейв («Port must be between 1 and 65535», `aria-invalid`)
  ...
  info-тултипы (Фаза 20.17.1) — абзац-хинт автозаписи не рендерится текстом; «ⓘ» —
  фокусируемая кнопка с `aria-label`, по фокусу появляется `role="tooltip"`, по blur
  исчезает.
