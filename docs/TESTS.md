...
- `filebrowser.ts` (Фаза 44.8) — чистое ядро общей файловой панели `FileBrowser` (SFTP + local
  как тонкие обёртки над ней). `buildVisibleItems` — окно виртуализации с рядом `..` (смещение на
  1 при наличии родителя определено в **одном** месте, а не двух копиях; проверены root/не-root,
  срез окна, отсутствие `undefined`-рядов за концом). `cursorForReturnedFolder` — куда встаёт
  курсор после «вверх» (на папку, из которой вышли, со смещением на `..`; фолбэк на первый ряд/-1).
  `pasteTargetName` — имя при вставке (cut сохраняет имя даже на коллизии; copy на коллизии
  дизамбигуирует «… copy»). `isDestExists` — распознавание маркера `dest-exists` в типизированной
  ошибке. Контракт `FileBrowserAdapter` (транспорт + навигационная семантика) — типами.
- `markdown.ts` — мини-рендер Markdown: `escapeHtml`,
...
- `slugify` + якоря заголовков — заголовки несут slug-`id` для оглавления встроенного
  мануала (эмодзи/пунктуация выкидываются, кириллица остаётся; дедуп повторов
  `git`/`git-2`; пустой slug (all-эмодзи) → заголовок **без** `id`; slug attribute-safe —
  ни кавычек, ни слэшей). TOC-ссылка `[…](#slug)` рисуется как якорь.
- `actions/mdlinks.test.ts` — клик по ссылке в отрендеренном markdown: внутренний
  `#slug` **скроллит** к заголовку с этим `id` внутри контейнера (не навигация); http(s)
  уходит в системный опенер; `javascript:` и несуществующий якорь — инертны, но
  `preventDefault` всегда (WebView не должен навигировать).
- `safeUrl` (Фаза 44.3) — allowlist схем для цели ссылки. Пропускает `http`/`https`/
  `mailto` и относительные (включая двоеточие **внутри пути**: `./a/b:c` — не схема);
  отклоняет `javascript:`/`vbscript:`/`data:`, протокол-относительный `//host` и пустую
  цель. Отдельным тестом — обходы, **которые браузер прощает, а наивная проверка префикса
  нет**: `java\tscript:`, `java\nscript:`, ведущие пробелы (WebKit/Chromium выбрасывают
  управляющие символы и пробелы при разборе схемы, поэтому такая строка исполняется как
  `javascript:`). Плюс поведение `inline` на отклонённой цели: якорь **не рисуется вовсе**,
  метка остаётся текстом и всё ещё экранируется (`[<b>hi</b>](javascript:x)` →
  `&lt;b&gt;hi&lt;/b&gt;`). Мёртвый `href="#"` был бы хуже: выглядит нажимаемым и ничего
  не объясняет.
- `mdlink.guard.test.ts` (Фаза 44.3, гейт) — каждый `{@html}` с markdown висит на элементе
  с `use:mdLinks`. Существует потому, что модуль писался под один доверенный вызов, а
  оброс пятью, из которых четыре кормили его выводом LLM / файлом с сервера / заметками —
  и перехват кликов остался ровно в том единственном, где вход и так доверенный. Проверка
  **поэлементная и по исходнику со срезанными комментариями**: первая версия гейта была
  файловой («встречается ли `use:mdLinks`») и **прошла на файле с удалённым экшеном**,
  потому что рядом лежал комментарий, упоминавший экшен словами — та же ловушка, что с
  доком `tabs.svelte.ts` в 44.2. Мутационно проверены все пять точек (AiChat,
  RecordingsPanel, EditorTab, NotesModal, HelpPanel): снятие экшена ловится в каждой, с
  указанием файла и строки. Вторым тестом гейт проверяет, что список `EXEMPT`
  (не-markdown `{@html}` — реестр иконок) не протух: файл существует и сток в нём остался,
  иначе исключение охраняло бы пустоту.
- **Картинки превью (Фаза 44.4).** `markdown.ts` — `inline`/`collectImages`: неразрешённая
  цель даёт плейсхолдер, а не `!` + ссылку (регрессия, ради которой всё затевалось);
  разрешённая — `<img>`; идиома бейджа `[![alt](img)](href)` собирается в ссылку **вокруг**
  картинки (раньше рвалась о вложенные скобки в label ссылки); `src`, не прошедший
  `^data:image/…;base64,`, отбрасывается **даже когда его передал вызывающий** (`https://…`,
  `javascript:`, `data:text/html`) — проверка дублируется намеренно; alt экранируется;
  картинка в бэктиках остаётся литералом; title в цели (`(a.png "Подпись")`) отрезается.
  `collectImages` возвращает каждую цель однажды и **пропускает код** (фенс и бэктики):
  цель внутри кода обсуждают, а не показывают, и читать по ней файл — уже действие по
  содержимому документа. `mdimage.ts` — `imageMime` (регистр, `?query`/`#frag`, отказ SVG),
  `classifyImage`: относительная/абсолютная цель → путь, `http(s)`/`//host`/чужой `data:`
  → `remote` (не «сломано» — недостижимо), неподдерживаемый тип и выход выше корня →
  `unsupported`, **Windows-документ** резолвится нативно (буква диска читается как схема
  URL наивной проверкой — именно эта ловушка отправила бы все Windows-картинки в `remote`).
  `fspath.ts` — `resolveRelative`: `.`/`..`, абсолютная цель игнорирует каталог, нативные
  спеллинги Windows/UNC, корень диска и шара не съедаются, климб выше корня → `null`
  (не клампится: клампинг разрешился бы в другой **существующий** файл — исход, похожий на
  успех). Rust — `localfile::read_bytes`: base64 сходится, потолок размера **отказывает,
  а не обрезает**, каталог не читается как картинка.
- **Бейджи shields.io (Фаза 44.5).** `badge.ts` — `parseStaticBadge`: три- и двухполевая
  форма, экранирование shields (`--`/`__`/`_`, %-декод), bare-hex, `?style`/`?color`/
  `?labelColor`, выживание после HTML-экранирования (`&amp;` в query). **Ядро тестов —
  граница «статический ↔ динамический»:** `/github/stars`, `/npm/v`, `/badge/dynamic/json`,
  `/gitlab/pipeline-status` → `null` (их значение на сервере, выдумывать нельзя); чужой хост
  (`shields.io.evil.test`) → `null`; непонятый цвет / одно поле → `null`. `badgeColor` —
  имена/алиасы/hex, и **отказ на инъекцию** (`"/><script>`, `red; x`) — цвет уходит в
  SVG-атрибут, allowlist его и стережёт. `badgeSvg` — обе половины с текстом и заливками,
  экранирование текста (крафт-метка не ломает разметку), **нет `id`** (несколько бейджей на
  странице не должны слипнуться геометрией первого), **нет `style`** (nonce в `style-src`
  убил бы его в упакованной сборке), flat-square/for-the-badge, тёмный текст на светлом фоне,
  message-only без левой половины. `markdown.ts` — бейдж рисуется вместо плейсхолдера,
  линкованный бейдж остаётся в якоре, динамический — плейсхолдер.
- **HTML-разметка в превью (Фаза 44.6).** `htmlsan.ts` (через `inline(x,{html:true})` и
  `renderMarkdown`): admit-набор (`<div align>`/`<sub>`/`<kbd>`/`<br>`, эмфазис на тексте
  вокруг тегов, экранирование текста между тегами). **Ядро — граница безопасности:**
  `<script>`/`<style>`/`<iframe>`/`<object>`/`<svg>`/`<form>` дропаются **с содержимым**
  (в т.ч. незакрытые — до конца, и самозакрытый `<svg/>` — без проглатывания хвоста);
  `on*`/`style`/`class`/`data-*` снимаются; `javascript:`/`vbscript:` в `href` → инертный
  якорь; неизвестный тег снимается, дети остаются; комментарии дропаются; стрэй `<`
  экранируется. **Батарея враждебных вводов** (`<img onerror>`, `<DIV ONCLICK>`,
  `<scr<script>ipt>`, `style=url(javascript:)`, `<svg/onload>`, `java\tscript:`,
  `<iframe srcdoc>`): в выводе нет живого `on…=`/`javascript:`/`<script`/`<svg`/`<iframe`/
  `style=`. Валидация значений: `align`/`valign`/`dir`/`scope`/`type` по словарям,
  `width`/`height` по цифрам/`%`, `colspan` по цифрам, free-text `title` экранируется.
  Картинки из raw-HTML `<img>` — тем же резолвером (badge/`data:`/плейсхолдер, размеры
  прокидываются). `htmlBlockTag` (блочный тег строки, `null` для инлайновых/не-HTML),
  `collectHtmlImages` (`<img src>`, игнор `<source>`), `protectHtml` (плейсхолдеры на
  admit, drop на прочее). `renderMarkdown` блочный: HTML-строка не в `<p>`, многострочный
  `<!--…-->` глотается, при **выключенном** `html` raw-HTML по-прежнему экранируется.
...
- `statuscolor.guard.test.ts` (Фаза 43, гейт) — ни одного сырого класса палитры Tailwind
  ...
  **Расширение Фазы 44.10:** третий тест — ни одного безчислового `text-white`/`text-black`
  (с опциональной прозрачностью) в `.svelte`, кроме строки со сплошной насыщенной кнопкой
  (`bg-green-500/600`, `bg-danger`, `bg-accent` — но **не** тинт `bg-accent/10`). Тот же баг,
  что и с нумерованными классами (белым по кремовому на светлой теме), только исходный гейт
  ловил лишь классы с номером и `text-white` просачивался. Четвёртый тест сторожит, что
  санкция белого-на-кнопке ещё кого-то находит.
