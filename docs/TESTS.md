...
18 файлов `*.guard.test.ts` — обычные vitest-тесты, которые читают **исходники** (а где контракт
живёт в бинарном артефакте — сам артефакт) и падают на
...
- **Упоминание зависимости — не гейт.** `dockpanels.guard` требовал, чтобы `$effect` поллинга
  содержал `visible`; на снятом условии он остался зелёным, потому что рядом уцелела строка
  `void visible;` (она существует только чтобы эффект перезапускался). Такие голые чтения
...
уничтожал каждый сохранённый файл: атрибуты собирались из шаблона russh-sftp, уходил
`ATTR_SIZE = 0`, сервер делал `truncate`, и пустой временный файл переезжал на место конфига.
Дефект жил не в вычисленном значении, а в том, **что мы попросили сделать сервер**.
