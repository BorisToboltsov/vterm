...
- **Логотип приложения** (бренд-марка) — минималистичный терминальный промпт **`>_`**
  ...
  One Dark `#21252b`); набор иконок ОС генерится из него `pnpm tauri icon`. In-app та же
  марка (напр. перед вступительным слоганом вкладки «Справка») рисуется инлайн-SVG на **тематических токенах**
  (`--color-panel`/`--color-edge`/`--color-accent`), чтобы совпадать с активной темой —
  это **единственное** исключение из «не инлайнить SVG» (сам логотип, не UI-иконка).
