...
3. **Прогон всего набора.** После задачи гоняй гейты только те которые были затронуты. (команды ниже). Красное — чини.
...
6. **Версия и три артефакта** — см. ниже.
...
**Порядок важен:** сперва `rm` бандла, потом скрипт — тогда build-`.app` уже не
существует и попадает под вычистку. Каждый `tauri build` монтирует `.dmg` в новый
`/Volumes/dmg.XXXX` и регистрирует лежащий внутри `vterm.app` в LaunchServices; `rm`
бандла делает эту запись фантомной. За десятки пересборок их набираются десятки — все с
bundle id `su.vcore.vterm`, — и macOS-Finder на «Открыть с помощью → vterm» резолвит id в
мёртвый путь: приложение молча не открывается. Скрипт (idempotent, только пути `vterm.app`,
только несуществующие) это чинит и переобъявляет `/Applications/vterm.app` авторитетным.
Это **дефект машины разработчика**, а не код: у конечного пользователя одна регистрация.
...
## Сборка: три артефакта на каждой фазе

Каждая фаза даёт **три** дистрибутива — macOS (`.app`/`.dmg`), Windows (`.msi`/`.exe`)
и Linux (`.deb`/`.rpm`/AppImage, x86_64). Единый кросс-ОС файл невозможен; Windows и
Linux на этом Mac нативно не собираются (кросс-компиляция Tauri между ОС нерабочая) →
механизм — **GitLab CI** ([.gitlab-ci.yml](.gitlab-ci.yml), self-hosted раннеры;
`build:macos` + `build:windows` + `build:linux`). CI: `lint → test → build → release`.
