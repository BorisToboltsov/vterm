...
// Именно этот дрейф и случился в жизни: set-version.mjs синкал три файла, а
// плашку версии в README не трогал — и бейдж молча застрял на 1.0.0, пока
// package.json уехал вперёд. Плашка статична (ничем не резолвится), поэтому
// единственная защита — синк скриптом + этот гейт.
describe("README-плашка версии не отстаёт от package.json", () => {
  it("бейдж shields.io несёт версию из package.json", () => {
    const m = read("README.md").match(
      /shields\.io\/badge\/version-(\d+\.\d+\.\d+)/,
    );
    expect(m, "плашка версии в README.md не найдена").not.toBeNull();
    expect(m![1]).toBe(pkgVersion);
  });
});
